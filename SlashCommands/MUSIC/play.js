...
module.exports = {
  ...
  run: async (client, interaction, args) => {
    ...
    if (loadType === "PLAYLIST_LOADED") {
      ...
      interaction.editReply({ content: `<a:disk:1081896564949254194> Added \`${resolve.tracks.length} from ${resolve.playlistInfo.name}\`` });
      ...
      interaction.editReply({
        embeds: [{
          color: 'PURPLE',
          description: `<a:disk:1081896564949254194> Added \`${track.info.title}\``
        }]
      });
      if (!player.isPlaying && !player.isPaused) return player.play();

  ...
  run: async (client, message, args) => {
    ...
    if (args[0] == "radio") {
      ...
      if (args[1] == "1") {
        ...
        player.queue.add(track); message.channel.send({ content: `<a:disk:1081896564949254194> Playing Radio: \`${track.info.title}\`` });
        if (player.isPlaying) {
          player.stop()
        } else {
          player.play()
      ...
      if (loadType === "PLAYLIST_LOADED") {
        ...
        message.channel.send({ content: `<a:disk:1081896564949254194> Added \`${resolve.tracks.length} from ${resolve.playlistInfo.name}\`` });
        ...
        message.channel.send({
          content: `<a:disk:1081896564949254194> Added  \`${track.info.title}\``
        });
        if (!player.isPlaying && !player.isPaused) return player.play();
